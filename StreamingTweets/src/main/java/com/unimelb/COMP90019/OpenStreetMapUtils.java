...
//        System.out.println("Query:" + query);
        ...
        if (obj instanceof JSONArray) {
            ...
            if (array.size() > 0) {
                ...
                ArrayList<Double> bodbox = new ArrayList<Double>();

                String lon = (String) jsonObject.get("lon");
                String lat = (String) jsonObject.get("lat");
                JSONArray JSONbodbox =  (JSONArray) jsonObject.get("boundingbox");

                if (JSONbodbox != null) {
                    int len = JSONbodbox.size();
                    for (int i=0;i<len;i++){
                        String strbodbox = JSONbodbox.get(i).toString();
                        bodbox.add(Double.parseDouble(strbodbox));
//                    System.out.println("dparse"+Double.parseDouble(strbodbox));
                    }
                }
...
//                System.out.println("lat=" + lat);
                ...
                res.put("lonmax",bodbox.get(1));
                res.put("latmin",bodbox.get(2));
                res.put("latmax",bodbox.get(3));
            }
        }
